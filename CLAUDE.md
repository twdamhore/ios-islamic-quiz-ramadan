...
- After requesting the review, poll the PR review status (using `gh pr view` and `gh api` to check for new reviews/comments) until Claude finishes the review. Also check for "This branch has conflicts that must be resolved" — if present, resolve the conflicts before continuing.
